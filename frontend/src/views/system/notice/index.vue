    ...
    <a-card :bordered="false">
      <s-table
        ...
        <template slot="operator" v-if="hasPerm('sysNotice:add')">
          <a-button @click="$refs.addForm.add()" icon="plus" type="primary" v-if="hasPerm('sysNotice:add')">新增公告
          </a-button>
        </template>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (this.hasPerm('sysNotice:changeStatus') || this.hasPerm('sysNotice:edit') || this.hasPerm(
        'sysNotice:delete')) {
        this.columns.push({
          title: '操作',
          width: '300px',
